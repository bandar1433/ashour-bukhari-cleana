...
export async function features(req:any,res:any,u:any){
  ...
  if(sub==='reports'){
    ...
    const rawStudents=await query<any>(`select s.id,s.full_name,s.center_id,s.circle_id,c.name center_name,h.name circle_name,
      ...
      from students s left join circles h on h.id=s.circle_id left join centers c on c.id=s.center_id
...
export async function profile(req:any,res:any,u:any){
  ...
  if(req.method==='PUT'){
    const b=req.body||{};
    const row=(await query<any>('update users set full_name=coalesce($2,full_name),phone=coalesce($3,phone),avatar_url=coalesce($4,avatar_url) where id=$1 returning id,full_name,email,phone,avatar_url,role::text role,center_id,is_active',[u.id,String(b.full_name||'').trim()||null,String(b.phone||'').trim()||null,String(b.avatar_url||'').trim()||null]))[0];
