...
export async function features(req:any,res:any,u:any){
  ...
  if(sub==='reports'){
    ...
    const students=await query<any>(`select s.id,s.full_name,s.center_id,s.circle_id,c.name center_name,h.name circle_name,
      ...
      coalesce((select round(100.0*coalesce(sum(m.page_count),0)/nullif(sum(case when coalesce(w.new_target,'')~'^\\d+(\\.\\d+)?
    ...
    const grouped=(key:'circle_id'|'center_id',nameKey:'circle_name'|'center_name')=>{
      const map=new Map<string,any>();for(const s of students){const id=String(s[key]||'none');let g=map.get(id);if(!g){g={id,name:s[nameKey]||'—',students:0,attendance_sum:0,grade_sum:0,achievement_sum:0,new_pages:0,review_pages:0,struggling:0};map.set(id,g)}g.students++;g.attendance_sum+=Number(s.attendance_rate||0);g.grade_sum+=Number(s.avg_grade||0);g.achievement_sum+=Number(s.achievement_rate||0);g.new_pages+=Number(s.new_pages||0);g.review_pages+=Number(s.review_pages||0);if(Number(s.attendance_rate)<75||Number(s.avg_grade)<70)g.struggling++}
    ...
    const n=students.length,metrics={total_students:n,attendance_rate:n?Math.round(students.reduce((x:number,s:any)=>x+Number(s.attendance_rate||0),0)/n):0,avg_grade:n?Math.round(students.reduce((x:number,s:any)=>x+Number(s.avg_grade||0),0)/n):0,achievement_rate:n?Math.round(students.reduce((x:number,s:any)=>x+Number(s.achievement_rate||0),0)/n):0,new_pages:students.reduce((x:number,s:any)=>x+Number(s.new_pages||0),0),review_pages:students.reduce((x:number,s:any)=>x+Number(s.review_pages||0),0),struggling:struggling.length,most_improved:mostImproved.length,top_performers:topPerformers.length};
...
export async function profile(req:any,res:any,u:any){
 ...
 then w.new_target::numeric else 0 end),0))::int
        from weekly_plans w left join memorization_records m on m.student_id=w.student_id and m.record_type='new' and m.record_date between $1 and $2
        where w.student_id=s.id and w.week_start between ($1::date-interval '6 days') and $2::date),0) achievement_rate,
      coalesce((select round(avg(m.grade))::int from memorization_records m where m.student_id=s.id and m.record_date between current_date-6 and current_date),0) current_avg,
      coalesce((select round(avg(m.grade))::int from memorization_records m where m.student_id=s.id and m.record_date between current_date-13 and current_date-7),0) previous_avg
      from students s left join circles h on h.id=s.circle_id left join centers c on c.id=s.center_id where s.status='active' and ${scope} order by c.name,h.name,s.full_name`,args);
    const grouped=(key:'circle_id'|'center_id',nameKey:'circle_name'|'center_name')=>{
