...
export async function dayApprove(req:any,res:any,u:any){
 ...
 then w.new_target::numeric else 0 end nt,
        case when coalesce(w.review_target,'')~'^\\d+(\\.\\d+)?
}
...
export async function dayApprove(req:any,res:any,u:any){
 ...
 then w.review_target::numeric else 0 end rt
      from days d left join weekly_plans w on w.student_id=d.student_id and w.week_start=d.week_start and w.day_name=d.day_name
    ), m as (
      select student_id,record_date,sum(case when record_type='new' then coalesce(page_count,0) else 0 end)::numeric np,
        sum(case when record_type='review' then coalesce(page_count,0) else 0 end)::numeric rp
      from memorization_records where record_date between $4::date and ($4::date+interval '5 days')::date group by student_id,record_date
    )
    select d.student_id,round(avg(case when a.status='excused' then null else
      (case when a.status='absent' then 0 when a.status in ('present','late') and coalesce(a.late_minutes,0)<=30 then 30 when a.status in ('present','late') and coalesce(a.late_minutes,0)<=60 then 20 when a.status in ('present','late') then 10 else 0 end)
      +(case when p.rt>0 then least(40,round(40*coalesce(m.rp,0)/p.rt)) else 0 end)
      +(case when p.nt>0 then least(30,round(30*coalesce(m.np,0)/p.nt)) else 30 end) end))::int weekly_score
    from days d left join p on p.student_id=d.student_id and p.record_day=d.record_day left join m on m.student_id=d.student_id and m.record_date=d.record_day left join attendance a on a.student_id=d.student_id and a.attendance_date=d.record_day group by d.student_id`,
    [u.role,u.center_id,u.id,weekStart]);
  const weeklyMap=new Map(weekly.map((x:any)=>[x.student_id,x.weekly_score]));
  return json(res,200,{date:d,week_start:weekStart,is_friday:isFriday,students:scoredStudents.map((x:any)=>({...x,weekly_score:weeklyMap.get(x.id)??null})),approvals});
}
