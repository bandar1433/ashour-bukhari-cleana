...
export async function selfService(req:any,res:any,u:any){
 ...
 if(req.method==='POST'){
  ...
  if(kind==='punch'){
   ...
   const d=today(),dateObj=new Date(d+'T00:00:00Z');if(dateObj.getUTCDay()===5)return json(res,403,{error:'يوم الجمعة إجازة ولا يوجد تسجيل حضور.'});
   if(await isWeekLocked(s.circle_id,d))return json(res,403,{error:'الأسبوع مقفل',message:'تم إقفال الأسبوع من الإشراف.'});
   const approved=(await query<any>('select id from day_approvals where circle_id=$1 and approval_date=$2::date',[s.circle_id,d]))[0];if(approved)return json(res,403,{error:'تم اعتماد اليوم',message:'تم اعتماد سجل اليوم ولا يمكن للطالب تعديله.'});
   ...
   else if(action==='check_out'&&!a.check_out_at)a=(await query<any>('update attendance set check_out_at=now(),recorded_by=$1 where id=$2 returning *',[u.id,a.id]));
   return json(res,200,a);
  }
  if(kind==='quran'){
   if(!s.circle_id||s.status!=='active')return json(res,400,{error:'يلزم طالب نشط مرتبط بحلقة'});
   const d=today(),dateObj=new Date(d+'T00:00:00Z');if(dateObj.getUTCDay()===5)return json(res,403,{error:'يوم الجمعة إجازة ولا يوجد تسجيل يومي.'});
   if(await isWeekLocked(s.circle_id,d))return json(res,403,{error:'الأسبوع مقفل',message:'تم إقفال الأسبوع من الإشراف.'});
   const approved=(await query<any>('select id from day_approvals where circle_id=$1 and approval_date=$2::date',[s.circle_id,d]))[0];if(approved)return json(res,403,{error:'تم اعتماد اليوم',message:'تم اعتماد سجل اليوم ولا يمكن للطالب تعديله.'});
   const b=req.body||{},recordType=String(b.record_type||'');if(!['new','review'].includes(recordType))return json(res,400,{error:'اختر المراجعة أو الحفظ الجديد.'});
   ...
   if(existing){const row=(await query<any>(`update memorization_records set surah_no=$1::smallint,from_ayah=$2::smallint,to_surah_no=$3::smallint,to_ayah=$4::smallint,from_page=$5::smallint,to_page=$6::smallint,page_count=$7::smallint,ayah_count=$8::smallint,recorded_by=$9 where id=$10 returning *`,[fromSurah,fromAyah,toSurah,toAyah,fromPage,toPage,pageCount,ayahCount,u.id,existing.id]))[0];return json(res,200,row)}
   const row=(await query<any>(`insert into memorization_records(student_id,record_type,surah_no,from_ayah,to_surah_no,to_ayah,from_page,to_page,page_count,ayah_count,grade,notes,qiraah,approved,record_date,recorded_by) values($1,$2,$3::smallint,$4::smallint,$5::smallint,$6::smallint,$7::smallint,$8::smallint,$9::smallint,$10::smallint,null,null,'حفص عن عاصم',false,$11::date,$12) returning *`,[s.id,recordType,fromSurah,fromAyah,toSurah,toAyah,fromPage,toPage,pageCount,ayahCount,d,u.id]))[0];return json(res,201,row);
  }
 }
