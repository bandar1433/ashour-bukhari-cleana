...
export default async function handler(req:any,res:any){
 ...
 try{
  if(req.method==='GET'){
   const rows=await query(`select c.id,c.name,c.location,c.manager_user_id,c.is_active,u.full_name manager_name,count(ci.id)::int circles_count from centers c left join users u on u.id=c.manager_user_id left join circles ci on ci.center_id=c.id group by c.id,u.full_name order by c.name`);
