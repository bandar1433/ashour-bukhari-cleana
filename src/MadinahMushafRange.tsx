...
const surahs=Array.from({length:114},(_,i)=>{const n=i+1;const info:any=getSurahInfo(n as any);return {n,name:info?.name?.arabic||info?.arabicName||info?.name||('سورة '+n),ayahs:getAyahCountInSurah(n as any)}});
...
export function MadinahMushafRange({initial,onChange}:{initial?:Partial<MushafRange>;onChange?:(v:MushafRange)=>void}){
 ...
 return <div className="mushafRange">
  <label className="field"><span>من سورة</span><select name="surah_no" value={fromS} onChange={e=>setFS(+e.target.value)}>{surahs.map(s=><option key={s.n} value={s.n}>{s.n} — {s.name}</option>)}</select></label>
  ...
  <label className="field"><span>إلى سورة</span><select name="to_surah_no" value={toS} onChange={e=>setTS(+e.target.value)}>{surahs.map(s=><option key={s.n} value={s.n}>{s.n} — {s.name}</option>)}</select></label>
