...
export default function StudentQuranProfile({data,month,onMonthChange,onRefresh}:{data:any;month:string;onMonthChange:(m:string)=>void;onRefresh:()=>Promise<void>}){
 const [edit,setEdit]=useState<any>(null),[busy,setBusy]=useState(false),[error,setError]=useState('');
 ...
 async function punch(action:'check_in'|'check_out'){setBusy(true);setError('');try{await apiPost('/api/ops?action=self-service&kind=punch',{action});await onRefresh()}catch(e){setError(e instanceof Error?e.message:'تعذر تسجيل الحضور')}finally{setBusy(false)}}
 async function qsave(e:any){e.preventDefault();if(!edit)return;setBusy(true);setError('');try{const body:any=Object.fromEntries(new FormData(e.currentTarget).entries());body.record_type=edit.kind;await apiPost('/api/ops?action=self-service&kind=quran',body);setEdit(null);await onRefresh()}catch(x){setError(x instanceof Error?x.message:'تعذر حفظ السجل')}finally{setBusy(false)}}
 const editable=(d:any)=>norm(d.record_date)===today&&!d.approved&&!d.week_locked;
 return <div className="studentQuranProfile">{error&&<div className="notice">{error}</div>}
  ...
  <div className="studentWeekGrid">{weekly.map((d:any)=><article className={'studentDayPlan '+progressClass(d.progress)} key={norm(d.record_date)}>
    ...
    <div className="planMetric"><span>المراجعة</span><b>{d.review_done||0}/{d.review_target_pages||0} ص</b><small>المتبقي {d.review_remaining||0} ص</small></div>
    <div className="planMetric"><span>الحفظ الجديد</span><b>{d.new_done||0}/{d.new_target_pages||0} ص</b><small>المتبقي {d.new_remaining||0} ص</small></div>
  ...
  <div className="table-wrap studentMonthlyRegister"><table><thead><tr><th>اليوم</th><th>التاريخ</th><th>الحضور والانصراف</th><th>المراجعة</th><th>الحفظ الجديد</th><th>الصفحات</th><th>التقييم الآلي</th><th>الحالة</th></tr></thead><tbody>{monthly.map((d:any)=>{const can=editable(d),pages=Number(d.review_done||0)+Number(d.new_done||0);return <tr key={norm(d.record_date)} className={norm(d.record_date)===today?'todayRow':''}><td>{d.day_name}</td><td>{norm(d.record_date)}</td><td>{can?<div className="studentPunchCell"><span>{d.status==='late'?'متأخر':d.status==='present'?'حاضر':d.status==='absent'?'غائب':d.status==='excused'?'مستأذن':'لم يسجل'}</span>{!d.check_in_at?<button className="primary" disabled={busy} onClick={()=>punch('check_in')}>حضور</button>:!d.check_out_at?<button className="secondary" disabled={busy} onClick={()=>punch('check_out')}>انصراف</button>:null}<small>{fmt(d.check_in_at)} — {fmt(d.check_out_at)}</small></div>:<><b>{d.status==='late'?'متأخر':d.status==='present'?'حاضر':d.status==='absent'?'غائب':d.status==='excused'?'مستأذن':'—'}</b><small>{fmt(d.check_in_at)} — {fmt(d.check_out_at)}</small></>}</td>
    <td><button className="studentQuranCell" disabled={!can} onClick={()=>setEdit({kind:'review',old:d.review})}><b>{quranText(d.review)}</b><small>الخطة {d.review_target_pages||0}ص • المنجز {d.review_done||0}ص{can?' • اضغط للتسجيل/التعديل':''}</small></button></td>
    <td><button className="studentQuranCell" disabled={!can} onClick={()=>setEdit({kind:'new',old:d.new_record})}><b>{quranText(d.new_record)}</b><small>الخطة {d.new_target_pages||0}ص • المنجز {d.new_done||0}ص{can?' • اضغط للتسجيل/التعديل':''}</small></button></td>
  ...
  {edit&&<div className="panel editPanel"><div className="panelHead"><div><span className="panelEyebrow">{edit.kind==='review'?'المراجعة':'الحفظ الجديد'}</span><h3>تسجيل اليوم — {today}</h3></div><button className="secondary" onClick={()=>setEdit(null)}>إغلاق</button></div><form className="quickForm" onSubmit={qsave}><MadinahMushafRange key={(edit.old?.id||today)+edit.kind} initial={edit.old||{}}/><button className="primary" disabled={busy}>{edit.old?'حفظ التعديل':'حفظ التسجيل'}</button></form></div>}
 </div>
}
