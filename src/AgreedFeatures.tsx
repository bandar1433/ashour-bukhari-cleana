...
export default function AgreedFeatures({mode,currentRole,students}:{mode:Mode;currentRole:string;students:StudentRow[]}){
  ...
  if(mode==='profile')return <div>{error&&<div className="notice">{error}</div>}{data&&<form className="quickForm" onSubmit={async e=>{e.preventDefault();try{await apiPut('/api/ops?action=profile',formBody(e));await load()}catch(x){setError(x instanceof Error?x.message:'تعذر الحفظ')}}}><label className="field"><span>الاسم الكامل</span><input name="full_name" defaultValue={data.full_name||''}/></label><label className="field"><span>رقم الجوال</span><input name="phone" defaultValue={data.phone||''}/></label><label className="field"><span>البريد</span><input value={data.email||''} disabled/></label><label className="field"><span>نوع الحساب</span><input value={data.role||''} disabled/></label><button className="primary">حفظ البيانات</button><button className="secondary" type="button" disabled title="يُفعّل مع خدمة الجوال">التحقق OTP — قريبًا</button></form>}</div>;
