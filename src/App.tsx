      ...
      {!code ? <>
        ...
        {publicView==='عن الحلقات'&&<AboutSections/>}{publicView==='الرئيسية'&&<>
          ...
          <section className="featureBand reveal reveal-up">
            <div><span>منظومة واحدة لكل المسيرة</span><h2>من أول تسميع إلى تقرير الأسرة</h2><p>تجمع المنصة الحضور والحفظ والمراجعة والحفظ والمراجعة والنقاط والتقارير في تجربة واحدة؛ لتصبح المعلومة أقرب والقرار أسرع والمتابعة أدق.</p><button className="featureCta" type="button" onClick={()=>{setAuthIntent('signin');setAuthOpen(true);setError('')}}>ابدأ من لوحة المنصة</button></div>
            <div className="featureList">{['تسجيل سريع للحفظ والمراجعة والحفظ والمراجعة','متابعة الحضور والحالة اليومية','نقاط وجوائز تحفّز الاستمرار','تقارير قابلة للطباعة والحفظ','صلاحيات مستقلة للإدارة والمعلم والأسرة'].map(item=><p key={item}>✓ {item}</p>)}</div>
          </section>
        </>}
