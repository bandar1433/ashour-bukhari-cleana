...
function CountUp({value,decimals=0,suffix=''}:{value:number;decimals?:number;suffix?:string}){const [shown,setShown]=useState(0);useEffect(()=>{const target=Number(value)||0;if(target===0){setShown(0);return}const start=performance.now(),duration=650;let frame=0;const tick=(now:number)=>{const p=Math.min(1,(now-start)/duration),ease=1-Math.pow(1-p,3);setShown(target*ease);if(p<1)frame=requestAnimationFrame(tick)};frame=requestAnimationFrame(tick);return()=>cancelAnimationFrame(frame)},[value]);return <>{shown.toLocaleString('ar-SA',{minimumFractionDigits:decimals,maximumFractionDigits:decimals})}{suffix}</>}
